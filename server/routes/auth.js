...
router.post('/login', async (req, res) => {
  ...
  try {
    const user = await db.get('SELECT * FROM users WHERE email = ? AND is_active = 1', [
      email.toLowerCase().trim(),
    ])

    if (!user || !bcrypt.compareSync(password, user.password)) {
      await db.run('INSERT INTO login_logs (email, success, ip) VALUES (?, 0, ?)', [email, ip])
      return res.status(401).json({ error: 'Invalid email or password' })
    }

    await db.run('INSERT INTO login_logs (user_id, email, success, ip) VALUES (?, ?, 1, ?)', [
      user.id,
      email,
      ip,
    ])
