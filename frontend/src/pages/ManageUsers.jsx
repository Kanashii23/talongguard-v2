  ...
  return (
    <div>
      ...
      <div className="relative">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400 select-none">
          🇵🇭
        </div>
        <input
          type="tel"
          ...
          maxLength={11}
          className={`w-full border rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 bg-gray-50 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 transition-all font-mono tracking-wider ${
            isDirty
              ? isValid
        ...
        <input
          ...
          className={`w-11 h-14 text-center text-2xl font-bold rounded-xl border-2 focus:outline-none transition-all ${
            value[i]
              ...
              : 'border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-white focus:border-forest-400'
          }`}
        />
      ))}
    </div>
...
function CreateUserModal({ onClose, onCreated, showToast }) {
  ...
  function validateStep() {
    ...
    if (step === 1) {
      ...
      if (!/^09\d{9}$/.test(form.phone)) {
        setError('Phone must be 09XXXXXXXXX (11 digits).')
        return false
      }
    }
    if (step === 3) {
      ...
      if (!codeVerified) {
        setError('Please verify the email with the code first.')
        return false
      }
    }
    if (step === 4) {
  ...
  function handleCodeChange(code) {
    ...
    if (code.length === 6) setCodeVerified(false)
  }
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg shadow-2xl max-h-[92vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Drag handle */}
        <div className="w-10 h-1 bg-gray-200 dark:bg-gray-700 rounded-full mx-auto mt-3 mb-1 sm:hidden flex-shrink-0" />

        {/* Header */}
        <div className="px-6 pt-4 pb-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 transition text-sm"
            >
              ✕
            </button>
          </div>
            ...
            {steps.map((s, i) => (
              <div key={s.num} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div
                    className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-bold transition-all ${
                      step > s.num
                        ...
                        : step === s.num
                          ? 'bg-forest-700 text-white shadow-md scale-110'
                  ...
                  <span
                    className={`text-[10px] mt-1 font-semibold ${step === s.num ? 'text-forest-700 dark:text-forest-400' : 'text-gray-400'}`}
                  >
                    {s.label}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 mb-4 rounded-full transition-all ${step > s.num ? 'bg-forest-400' : 'bg-gray-100 dark:bg-gray-700'}`}
                  />
                )}
              </div>
            ))}
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1">
          ...
          {step === 1 && (
            <div className="space-y-4" style={{ animation: 'fadeUp 0.25s ease forwards' }}>
              ...
              <div className="bg-blue-50 dark:bg-gray-800 border border-blue-100 dark:border-gray-700 rounded-xl px-4 py-3">
                <p className="text-blue-700 dark:text-blue-400 text-xs font-semibold mb-1">
                  📱 Why we need your phone number
                </p>
                <p className="text-blue-600 dark:text-gray-300 text-xs">
                  Used for account recovery if email access is lost. Must be a valid Philippine
            ...
            <div className="space-y-4" style={{ animation: 'fadeUp 0.25s ease forwards' }}>
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-forest-500 focus:ring-2 focus:ring-forest-100 bg-gray-50 dark:bg-gray-800 dark:text-white transition-all"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-forest-500 focus:ring-2 focus:ring-forest-100 bg-gray-50 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 transition-all resize-none"
                />
              </div>
            </div>
          )}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-forest-500 focus:ring-2 focus:ring-forest-100 bg-gray-50 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 transition-all disabled:opacity-60"
                  />
                  <button
                ...
                <p className="text-xs text-gray-400 mt-1.5">
                  A 6-digit code will be sent to this email to verify it's valid.
                </p>
              </div>
              ...
              {codeSent && (
                <div style={{ animation: 'fadeUp 0.2s ease forwards' }}>
                  ...
                  <p className="text-xs text-gray-400 text-center mt-2">
                    Check inbox and spam of <strong>{form.email}</strong>
                  </p>
                    ...
                    <div className="mt-4 bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-green-600 text-xs">
                          {form.email} is confirmed. Continue to set the password.
                        </p>
                      </div>
                    </div>
                  )}
          ...
          {step === 4 && (
            <div className="space-y-4" style={{ animation: 'fadeUp 0.25s ease forwards' }}>
                ...
                {form.password && (
                  <div className="flex items-center gap-2 mt-2">
                    <div className="flex-1 h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-300 ${strengthColors[passStrength]}`}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-3 pr-11 text-sm focus:outline-none focus:ring-2 bg-gray-50 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 transition-all ${
              ...
              {/* Final summary */}
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 text-sm space-y-1.5">
        ...
        {/* Footer */}
        <div className="px-6 pb-6 pt-3 border-t border-gray-50 dark:border-gray-800 flex gap-3 flex-shrink-0">
          {step > 1 && (
            <button
              onClick={prevStep}
              className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
            >
              ← Back
            </button>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl text-sm focus:outline-none focus:border-forest-400 dark:text-white dark:placeholder-gray-500 focus:ring-2 focus:ring-forest-50 transition shadow-sm"
          />
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-16 text-gray-400">
              <span className="w-6 h-6 border-2 border-gray-200 border-t-forest-500 rounded-full animate-spin mr-3" />
              Loading...
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              ...
              {!search && (
                <p className="text-sm mt-1">Click "Create Account" to add the first one.</p>
              )}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((u) => (
                    <tr
                      key={u.id}
                      className={`border-t border-gray-50 dark:border-gray-700/50 hover:bg-gray-50/60 dark:hover:bg-gray-700/30 transition-colors ${!u.isActive ? 'opacity-50' : ''}`}
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white flex-shrink-0 ${u.role === 'admin' ? 'bg-gradient-to-br from-eggplant-600 to-eggplant-800' : 'bg-gradient-to-br from-forest-600 to-forest-800'}`}
                          >
                      ...
                      <td className="px-5 py-3.5 hidden sm:table-cell">
                        {u.municipality ? (
                          <div>
                            ...
                            {u.barangay && (
                              <div className="text-gray-400 text-xs">{u.barangay}</div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${u.role === 'admin' ? 'bg-eggplant-100 text-eggplant-700' : 'bg-forest-100 text-forest-700'}`}
                        >
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${u.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${u.isActive ? 'bg-green-500' : 'bg-gray-400'}`}
                          />
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 dark:text-gray-300 hover:border-forest-400 hover:text-forest-700 transition disabled:opacity-50"
