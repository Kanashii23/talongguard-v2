...
export default function ResetPassword({ isLoggedIn }) {
  ...
  // If no token in URL, show invalid message
  useEffect(() => {
  ...
  const strength = newPass.length === 0 ? 0
    : newPass.length < 8 ? 1
    : /[A-Z]/.test(newPass) && /[0-9]/.test(newPass) && /[^A-Za-z0-9]/.test(newPass) ? 3 : 2
          ...
          <div className="p-8 sm:p-10">
            ...
            {success ? (
              <div className="text-center py-4 animate-fade-up">
                ...
                <p className="text-gray-400 text-sm mb-6">Your password has been updated successfully. Redirecting to login...</p>
                <div className="w-8 h-8 border-2 border-forest-200 border-t-forest-600 rounded-full animate-spin mx-auto" />
              </div>
            ) : (
              <>
                <div className="text-center mb-8">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-forest-700 to-eggplant-700 flex items-center justify-center text-2xl mx-auto mb-4 shadow-md">🔐</div>
                ...
                {error && (
                  <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm mb-5 flex items-center gap-2">
                    <span>⚠️</span> {error}
                    {!token && (
                      <Link to="/forgot-password" className="ml-auto text-red-600 underline font-medium text-xs whitespace-nowrap">
                        Request new link
                      </Link>
                    )}
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        disabled={!token}
                        className="w-full border border-gray-200 rounded-xl px-4 py-3 pr-11 text-sm focus:outline-none focus:border-forest-500 focus:ring-2 focus:ring-forest-100 bg-gray-50 transition-all disabled:opacity-50" />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      disabled={!token}
                      className={`w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 bg-gray-50 transition-all disabled:opacity-50 ${
                ...
                <div className="text-center mt-6">
                  <Link to="/login" className="text-sm text-gray-400 hover:text-forest-600 transition">← Back to Login</Link>
                </div>
              </>
            )}
          </div>
