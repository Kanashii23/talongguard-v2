...
function MiniCalendar({ records, selectedDate, onSelectDate }) {
  ...
  const dataDates = useMemo(
    () => new Set(records.map((r) => (r.date || r.scanned_at || '').split(' ')[0]).filter(Boolean)),
    [records]
  )
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
      <div className="flex items-center justify-between bg-forest-700 text-white px-4 py-3">
        <button
          onClick={() => nav(-1)}
          className="w-7 h-7 rounded-lg hover:bg-forest-600 flex items-center justify-center text-lg font-bold transition"
        >
          ‹
        </button>
          ...
          {cells.map((d, i) => {
            ...
            return (
              ...
              >
                ...
                {hasData && (
                  <span
                    className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isSelected ? 'bg-white' : 'bg-forest-500'}`}
                  />
                )}
              </button>
            )
    ...
    <div className="flex flex-col gap-6 p-5">
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          {[
            { val: 'streets', lbl: 'OpenStreetMap' },
            { val: 'satellite', lbl: 'Satellite' },
          ].map(({ val, lbl }) => (
            <label
              key={val}
            ...
            >
              <input
                ...
                name="tile"
                value={val}
                checked={tileType === val}
                onChange={() => setTileType(val)}
...
export default function Dashboard({ records, setRecords, isLoggedIn, showToast }) {
  ...
  const activeSessionBrgy = activeMapDate
    ? activeMapDate.split('||')[1] || 'Selected Session'
    : null
  ...
  const { lineData, trendInfo } = useMemo(() => {
    ...
    if (timelineFilter === 'all') {
      ...
      const data = dates.map((d) =>
        filtered
          .filter((r) => r.date.startsWith(d))
          .reduce((a, r) => a + (parseInt(r[timelineFilter]) || 0), 0)
      )
      datasets = [
        {
          label: cfg.label,
    ...
    if (totalByDate.length >= 2) {
      ...
      if (prev === 0 && recent > 0) {
        ...
        if (pct > 10)
          ...
          trendInfo = { arrow: '↓', label: `${Math.round(pct)}%`, color: 'text-green-600' }
  ...
  const healthy = activeFilters.has('healthy')
    ? statsRecords.reduce((a, r) => a + (parseInt(r.healthy) || 0), 0)
    : 0
  const insect = activeFilters.has('insect')
    ? statsRecords.reduce((a, r) => a + (parseInt(r.insect) || 0), 0)
    : 0
  const leafspot = activeFilters.has('leafspot')
    ? statsRecords.reduce((a, r) => a + (parseInt(r.leafspot) || 0), 0)
    : 0
  const mosaic = activeFilters.has('mosaic')
    ? statsRecords.reduce((a, r) => a + (parseInt(r.mosaic) || 0), 0)
    : 0
  const wilt = activeFilters.has('wilt')
    ? statsRecords.reduce((a, r) => a + (parseInt(r.wilt) || 0), 0)
    : 0
            ...
            {[
              ...
              >
                ...
                <div className="text-center sm:text-left">
                  ...
                  <div className="text-gray-400 text-[10px] sm:text-xs mt-0.5 leading-tight">
                    {lbl}
                  </div>
                </div>
              </div>
                        ...
                        >
                          <td className="px-4 sm:px-5 py-3 text-gray-600 text-xs sm:text-sm">
                            <div className="flex flex-col gap-1">
                              ...
                              {(r.is_edited === 1 || r.is_edited === true) && (
                                <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200 w-fit">
                                  ✏️ Edited
                                </span>
                              )}
                            </div>
                            ...
                            <td className="px-4 sm:px-5 py-3">
                              <div className="flex gap-1.5 sm:gap-2">
                                ...
                                <button
                                  ...
                                  className="text-xs px-2 sm:px-3 py-1.5 rounded-lg border border-red-100 text-red-500 hover:bg-red-50 transition"
                                >
                                  Del
                                </button>
                              </div>
            ...
            {tableRecords.length > pageSize && (
              <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-t border-gray-100">
                ...
                <button
                  onClick={() =>
                    setCurrentPage((p) =>
                      Math.min(Math.ceil(tableRecords.length / pageSize), p + 1)
                    )
                  }
