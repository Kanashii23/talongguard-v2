    ...
    <div className="page-enter">
      ...
      <section className="relative min-h-[calc(100vh-64px)] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-20"
          style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #22c55e 0%, transparent 50%), radial-gradient(circle at 80% 20%, #a855f7 0%, transparent 50%)' }}
        />
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="animate-fade-up">
            ...
            <p className="text-gray-300 text-lg leading-relaxed max-w-md mb-10">
              TalongGuard is an intelligent eggplant disease diagnostic rover equipped with a portable imaging system — autonomously scanning crops, detecting diseases, and mapping outbreaks to help farmers act fast and protect their harvest.
            </p>
          ...
          <div className="hidden lg:block animate-fade-up" style={{ animationDelay: '0.15s', opacity: 0 }}>
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-7 shadow-2xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-forest-500/30 flex items-center justify-center text-xl">🌿</div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {Object.entries(DISEASE_CONFIG).map(([key, cfg]) => (
                  <div key={key} className="flex items-center gap-2.5 bg-white/10 rounded-xl px-3.5 py-2.5">
                    <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: cfg.color }} />
                    <span className="text-white/80 text-xs font-medium leading-tight">{cfg.label}</span>
                  </div>
                ))}
              </div>
              <div className="mt-5 grid grid-cols-3 gap-3 border-t border-white/10 pt-5">
                {[
                  ...
                  <div key={lbl} className="text-center">
                    <div className={`font-display text-2xl font-bold text-white ${color || ''}`}>{val}</div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center mb-14">
          <p className="text-forest-600 text-sm font-semibold uppercase tracking-widest mb-3">What TalongGuard Does</p>
          <h2 className="font-display text-4xl font-bold text-forest-950">Built for the Field</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ icon, title, desc }) => (
            <div key={title} className="bg-white rounded-2xl p-7 border border-gray-100 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group">
